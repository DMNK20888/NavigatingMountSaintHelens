...
from AIModule import StupidAI, Dijkstras, AStarExp, AStarDiv, AStarMSH
from Map import Map


def findPath(enteredAI, startInput, goalInput):
    ...
    parser.add_argument('-AI', default='AStarExp', type=str, help='AI agent to use. Use any of the following: [AStarExp, AStarDiv, AStarMSH, Dijkstra]')
    ...
    agents = {'AStarExp': AStarExp, 'AStarDiv': AStarDiv, 'AStarMSH': AStarMSH, 'Dijkstra': Dijkstras, 'StupidAI':StupidAI}

    m = Map(w,l, start=startInput, goal=goalInput, seed=seed, filename=filename, cost_function=cost)
    alg = agents[ai]()
    t1 = time()
    ...
    totalTime = t2-t1
    print('Time (s): ', t2-t1)
    pathCost = m.createImage(path)

    return totalTime, pathCost[0], pathCost[1] 